...
using CSV, DataFrames, ScikitLearn, MolecularGraphKernels, CairoMakie, ColorSchemes, GraphViz, RDKitMinimalLib, PlutoUI, JLD2, Statistics, Random
...
# ╔═╡ e6703f44-c252-4eca-bcd6-505966195a6e
begin
	url = "https://raw.githubusercontent.com/SimonEnsemble/intro_to_data_science_fall_2022/main/hw/dt_fruity_smell/fruity.csv"
	download(url, joinpath("Decision_tree", "fruity.csv"))
	odor_data = CSV.read("Decision_tree\\fruity.csv", DataFrame)
end
...
length(unique(odor_data[:, "molecule"]))
...
combine(groupby(odor_data, "fruity odor"), :molecule => length => :length)
...
X = JLD2.load("Decision_tree\\maccs_fps_and_molecules.jld2")["X"]
...
begin
	local fig =Figure()
	local ax = Axis(fig[1, 1], xlabel="Number of molecule", ylabel ="Max fingerprint")
	heatmap!(X)
	Colorbar(fig[1, 2])
	fig
end
...
begin
	len = size(X)[1]
	dist_mol = [sum(X[i, :]) for i in range(1, len)]
end

# ╔═╡ 994ffc2a-2e50-4f9b-a664-72a58d6f3bbc
begin
	local fig =Figure()
	local ax = Axis(fig[1, 1], xlabel="# of bits activated", ylabel ="# of Molecule")
	hist!(dist_mol)
	ylims!(0,nothing)
	fig
end
...
begin
	len_bit =size(X)[2]
	dist_bit = [sum(X[:, i]) for i in range(1, len_bit)]
	local fig =Figure()
	local ax = Axis(fig[1, 1], xlabel="# of molecules where bits are active", ylabel ="# of bits")
	hist!(dist_bit)
	ylims!(0, nothing)
	fig
end
...
# ╔═╡ 4ca14c76-7c64-415e-9729-eadd0c4bdd82
ids_train, ids_test = partition(1:len, 0.75, shuffle=true, stratify=y)

# ╔═╡ 6484e4e0-8a22-4249-b2a5-34b57ffc7660
sum(y[ids_test])

# ╔═╡ b64a361d-5025-49bd-a481-59e6e83e2dd0
sum(y[ids_train])
...
# ╔═╡ cd709479-881a-4543-acc0-1af33703994e
X[ids_test, :]
...
X_train, y_train, X_test, y_test = (X[ids_train, :], y[ids_train], X[ids_test, :],
									y[ids_test])
...
sum(y[ids_train])/length(ids_train)

# ╔═╡ d3bdc700-be19-4616-8e64-1eda2f42ddad
sum(y[ids_test])/length(ids_test)
...
* `max_depth`: This tells the algorithm when to stop growing the tree
* `criterion`: This is the degree of how pure you want your splits to be
* `min_samples_split`: This is used to set the minimum number of samples required to split an internal node
* `min_samples_leaf`: This is minimum number of samples required to be at a leaf node
"
...
# ╔═╡ b9035c42-c7c3-4b4c-a1f0-9d43885ef1a7
md"""
* The test score is more indicative of how the model will perform on unseen data
"""
...
ŷ = algo.predict(X_test)

# ╔═╡ 06ff735a-0e5a-4a2d-bad5-8831d7d62b79
sum([yᵢ==ŷᵢ for (yᵢ, ŷᵢ) in zip(y_test, ŷ)])/length(ids_test)
...
begin
	...
	counter = 0
	for i in range(1, length(ids_test))
		if ŷ[i]==1 && y_test[i]==1
			counter+=1
		end
	end
	precision = counter/ sum(ŷ.==1)		
end
...
counter_recall = 0
	...
	recall = counter_recall/ sum(y_test.==1)		
end
...
# ╔═╡ 860b82f9-6c39-42f2-ba78-3c5b55e0325d
cm = [sum(y_test.==0 .&& ŷ.==0) sum(ŷ.==1 .&& y_test.==0);
	  sum(y_test.==1 .&& ŷ.==0) sum(ŷ.==1 .&& y_test.==1)]
...
ŷ_baseline = [rand((0, 1)) for i in range(1, length(ids_test))]

# ╔═╡ 38f88f6a-c751-4328-b4f0-f3104ba7fdd7
cm_baseline = [sum(y_test.==0 .&& ŷ_baseline.==0) sum(ŷ_baseline.==1 .&& y_test.==0);
	  sum(y_test.==1 .&& ŷ_baseline.==0) sum(ŷ_baseline.==1 .&& y_test.==1)]

# ╔═╡ 96da02dd-d37a-4c26-a10e-79434c9e3d3e
viz_confusion(cm_baseline)
...
# ╔═╡ 31f47fb6-3644-44e6-9dcf-ee6872724d63
cv_ids = train_test_pairs(stratified_cv, 1:length(y_train), y_train)
...
* what is the optimal `max_depth`? The optimal depth is 8
"

# ╔═╡ 883a4e5a-02a6-4e25-87f9-a30858811fcc
max_depth = collect(1:20)

# ╔═╡ 0be4ead7-36c2-45a3-a8d5-4e07379096a4
begin
	kf_accuracy = zeros(length(max_depth))
	...
	for (d, depth) in enumerate(max_depth)
		...
		for (ik, (id_train, id_val)) in enumerate(cv_ids)
			algo.fit(X_train[id_train, :], y_train[id_train])
			scores[ik] = algo.score(X_train[id_val, :], y_train[id_val])
		end
		kf_accuracy[d] = mean(scores)
	end
end

# ╔═╡ 2c8309cc-ad24-4bea-9892-53ce89712f0f
begin
	local fig = Figure()
	local ax = Axis(fig[1,1], xlabel="Tree depth", ylabel="Accuracy")
	lines!(max_depth, kf_accuracy)
	fig
end
...
begin
	compare = 0
	depth=0
	for (i, acc) in  enumerate(kf_accuracy)
		if acc > compare
			compare = acc
			depth = i
		end
...
# ╔═╡ 0bd94067-04b1-4b3c-8f8f-199b7417d805
depth
...
* The accuracy was Increased, but the performance metric needs to be clearer. Are we valuing recall more than the precision or vice verse?"


# ╔═╡ 7daad1fc-fe82-4de7-8066-a196ea3c096e
begin
	algo_opt = DecisionTreeClassifier(max_depth = depth)
	algo_opt.fit(X_train, y_train)
	algo_opt.score(X_test, y_test)
end

# ╔═╡ 1943a4aa-df4a-45bd-9804-1901fa361295
ŷ_opt = algo_opt.predict(X_test)

# ╔═╡ 7cbef9fb-6a76-47d7-8aa3-2e5819223f74
cm_opt = [sum(y_test.==0 .&& ŷ_opt.==0) sum(ŷ_opt.==1 .&& y_test.==0);
	  sum(y_test.==1 .&& ŷ_opt.==0) sum(ŷ_opt.==1 .&& y_test.==1)]

# ╔═╡ 674c78b8-8d17-438f-afd8-ee6f438d4c87
viz_confusion(cm_opt)
...
begin
	import PyPlot
	f = PyPlot.figure()
	plot_tree(algo_opt, class_names=["not fruity", "fruity"], filled=true)
...
* Probably not, because if new compounds that do not have any of the feature of the training set is developed, the algorithm may perform badly 
"
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "4212b3c1049ecf05f0ea76e92d59aa35c209c051"
